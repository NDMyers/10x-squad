...
- Assignment values are exact executable identifiers for that surface. `auto`, `inherit` (any casing), blank strings, `null`, non-strings, and unknown tier keys are invalid.
...
- Free-text identifiers are preserved byte-for-byte and remain `unverified` until the active harness verifies them.
- `model_checks` is optional advisory metadata keyed by the exact assignment value; entry fields are limited to `display_name`, `status` (`verified` | `unverified`), `method`, `source`, `checked_at`. Suggested sources: `harness`, `candidate_scan`, `manual`. There is no time-based expiry; `checked_at` is informational.
