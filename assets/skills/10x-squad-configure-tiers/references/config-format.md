...
- Every `dispatch_settings` entry contains exactly `reasoning_effort` and `context_tier`. Reasoning accepts only `auto|low|medium|high|xhigh`; context accepts only `auto|default|long_context`.
- In schema v2, `dispatch_settings` may be omitted only on a retained unrelated legacy profile. Upserting one harness preserves unrelated legacy profile objects byte-for-byte in meaning and does not add the map to them.
...
- Explicit runtime settings are allowed only for `copilot-cli`. `copilot-vscode` and unknown harness profiles may use `auto`/`auto`; if either setting is explicit, configuration hard-stops before any probe, preview, or write.
